...
public class CULLifecycleManager {
    ...
    public void config(Dictionary<String, ?> allConfig) {
        if (allConfig != null) {
            String deviceName = (String) allConfig.get(KEY_DEVICE_NAME);
            if (StringUtils.isEmpty(deviceName)) {
                throw new RuntimeException(KEY_DEVICE_NAME, "The device name can't be empty");
            }

            Matcher deviceMatcher = DEVICE_PATTERN.matcher(deviceName);
            if (!deviceMatcher.matches()) {
                throw new RuntimeException(KEY_DEVICE_NAME,
            ...
            if (configFactory == null) {
                throw new RuntimeException(KEY_DEVICE_NAME, "Invalid device type '" + deviceType + "'");
            }
